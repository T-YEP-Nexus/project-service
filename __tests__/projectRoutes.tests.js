...
const projectRoutes = require('../src/routes/projects/project.js');
const projectMiscRoutes = require('../src/routes/projects/misc/misc.js');
const projectResourceRoutes = require('../src/routes/projects/ressource/ressource.js');
...
describe('Project Routes (Integration)', () => {
  ...
  describe('POST /projects - Create project', () => {
    it('should create a project successfully', async () => {
      const newProject = {
        ...
        ressources: [{ url: 'https://example.com/doc.pdf', filename: 'doc.pdf' }],
        is_active: true,
    ...
    it('should fail with 400 if required fields are missing', async () => {
      const res = await request(BASE_URL).post('/projects').send({ name: 'BadProject' });
  ...
  describe('GET /projects/creator/:id_creator', () => {
    it('should return projects for a creator', async () => {
      const res = await request(BASE_URL).get(`/projects/creator/${testCreatorId}`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  });
  ...
  describe('GET /projects/promotion/:id_promotion', () => {
    it('should return projects for a promotion', async () => {
      const res = await request(BASE_URL).get(`/projects/promotion/${testPromotionId}`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  ...
  describe('PATCH /projects/:id - Update project', () => {
    ...
    it('should return 400 if no fields provided', async () => {
      const res = await request(BASE_URL).patch(`/projects/${testProjectId}`).send({});
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('Resources Management', () => {
    ...
    it('should get project resources', async () => {
      const res = await request(BASE_URL).get(`/projects/${testProjectId}/resources`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  });
  ...
  describe('PATCH /projects/:id/toggle-active', () => {
    it('should toggle project active status', async () => {
      const res = await request(BASE_URL).patch(`/projects/${testProjectId}/toggle-active`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  });
